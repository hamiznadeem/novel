...
<section class="py-16 lg:py-20 bg-gray-50 min-h-screen">
    <div class="container mx-auto px-4 lg:px-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
            <div class="space-y-2">
                <h3 class="text-3xl lg:text-5xl font-black text-[#0F172A]">Islamic —</h3>
                <p class="text-gray-500 font-medium italic">Discover our curated collection of Islamic literature and stories.</p>
            </div>
            ...
            <form method="GET" action="" class="relative w-full md:max-w-md group">
                ...
                <?php if($search_query): ?>
                    <a href="?" class="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black uppercase text-gray-400 hover:text-red-500 tracking-tighter transition-colors">Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 lg:gap-10">
            <?php
            if ($search_query !== '') {
                $sql = "SELECT * FROM novels WHERE category = 'islamic' AND isActive = 1 AND (name LIKE ? OR author LIKE ?)";
                $stmt = $conn->prepare($sql);
                $search_param = "%" . $search_query . "%";
                $stmt->bind_param("ss", $search_param, $search_param);
            } else {
                $sql = "SELECT * FROM novels WHERE category = 'islamic' AND isActive = 1 ORDER BY novel_id DESC";
                $stmt = $conn->prepare($sql);
            }
            ...
            if ($result->num_rows > 0) {
                ...
                <div class="col-span-full py-24 text-center">
                    <div class="bg-white inline-block p-12 rounded-[3rem] shadow-sm border border-dashed border-gray-200">
                        ...
                        <p class="text-gray-500 font-medium">We couldn't find any active Islamic novels matching "<?= htmlspecialchars($search_query) ?>"</p>
